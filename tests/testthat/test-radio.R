...
test_that("choices argument", {
  expect_error(radioInput("ID", LETTERS[1:3], 1))
  expect_error(radioInput("ID", values = 1:3))

  expect_silent(radioInput("ID", choices = LETTERS[1:3], values = 1:3))
})

test_that("selected argument", {
  expect_error(radioInput("ID", choices = LETTERS[1:3], selected = 1:2))

  expect_silent(radioInput("ID", choices = LETTERS[1:3], values = 1:3, selected = 3))
})
